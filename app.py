...
def make_trend_fig(daily, T):
    ...
    fig = make_subplots(
        ...
        subplot_titles=(T["t_prod"], T["t_rate"]),
    )
...
def make_defect_chart(daily, T):
    return make_trend_fig(daily, T)
...
def make_mode_compare_fig(mode_info, T):
    ...
    fig.update_layout(
        title=T["t_mode"].format(v=mode_info.get("split_variable", "")),
        height=360,
        xaxis=dict(showgrid=False),
        yaxis=dict(showgrid=True, gridcolor=COLOR_GRID, zeroline=False, ticksuffix="%"),
        **CHART_BASE_LAYOUT,
...
def render_answer_charts(spec, data, T):
    ...
    if spec[0].get("kind") == "overview":
        ...
        render_chart_card(make_trend_fig(get_daily_defect_rate(data), T))
    ...
    for item in spec:
        kind = item.get("kind")
        if kind == "trend":
            figs.append(make_trend_fig(get_daily_defect_rate(data), T))
        elif kind == "mode":
